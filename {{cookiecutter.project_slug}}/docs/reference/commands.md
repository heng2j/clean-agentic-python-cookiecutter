...
| `… gauntlet full` | offline after install | fast + coverage/CRAP/source scan/distribution build complete |
...
| `… gauntlet connected-audit` | connected | the pinned audit command completed and its mutable result was preserved; freshness is not implied |
...
Exit `0` means the named checks ran and met their declared oracles. For a
gauntlet, exit `1` means an executed child command returned nonzero; that
child's exact code remains in the ledger. Exit `2` means the harness itself
could not execute a configured step because of invalid configuration, a
missing or inaccessible executable, or timeout. Individual `cleanai`
subcommands may use exit `2` for invalid evidence or an unsafe precondition.
The JSON ledger is the stable machine interface; console prose is for people.
